...
export class MongodbOrderRepository implements IOrderRepository {
  async insertDraftOrder(draftOrder: DraftOrder): Promise<DraftOrder> {
    ...
    const find = await mongodbService.draftorder.findOne({
  ...
  async getAllOrderCancelled(): Promise<Order[]> {
    const orders = await mongodbService.order
      .find({ status: OrderStatus.CANCELLED })
      .toArray();
    return orders as Order[];
  }
  ...
  // async getAllOrderShippedBetweenTime(
  //   startDate: Date,
  //   endDate: Date
  // ): Promise<Order[]> {
  //   const orders = await mongodbService.order
  ...
  //     const orders = await mongodbService.order
  //       .find({ shipperId: new ObjectId(shipperId) })
  //       .toArray();
  //     return orders as Order[];
  //   }
  ...
  async takeOrderToDelivered(
    ...
    const result = await mongodbService.order.updateOne(
      { _id: new ObjectId(id), status: OrderStatus.PROCESSING },
      { $set: { status: OrderStatus.DELIVERED, delivered_at: new Date() } }
    );
  ...
  async generalStatistic(): Promise<any> {
    ...
    const revenueByPayment = await orders
      .aggregate([
    ...
    const totalProductsSold = await orders
      .aggregate([
        { $match: { status: OrderStatus.COMPLETED } },
        {
          $group: {
    ...
    const totalUsers = await orders.distinct("userId", {
      status: OrderStatus.COMPLETED,
    });
  ...
  async statisticByStatus(): Promise<any> {
    const statusStats = await mongodbService.order
      .aggregate([
        { $match: { status: OrderStatus.COMPLETED } },
  ...
  async getTopSellingProductItem(): Promise<any> {
    ...
    const topByQuantityRaw = await orderDetails
      .aggregate([
        {
          $lookup: {
            from: "order",
    ...
    // -------- TOP BY REVENUE --------
    const topByRevenueRaw = await orderDetails
      .aggregate([
        {
          $lookup: {
            from: "order",
  ...
  async getBrandStatistics(): Promise<any> {
    ...
    const stats = await orderDetails
      .aggregate([
