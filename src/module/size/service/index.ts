...
export class SizeService implements ISizeService {
  ...
  async create(form: ICreateSizeForm): Promise<Size> {
    ...
    const newSize = createSizeSchema.parse(fixedForm);
  ...
  async getSizeBySizeTypeId(sizeTypeId: string): Promise<Size[]> {
    const result = await this.sizeRepository.findSizeBySizeTypeId(sizeTypeId);
    return result;
  }
  async getSizeBySizeTypeName(sizeTypeName: string): Promise<Size[]> {
    const result = await this.sizeRepository.findSizeBySizeTypeName(
      sizeTypeName
    );
    return result;
  }
