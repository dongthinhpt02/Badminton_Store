...
export class CartItemService implements ICartItemService {
  ...
  async create(form: CreateCartItem): Promise<CartItem> {
    ...
    if (existingCartItem) {
      ...
      const productInCartItem = await mongodbService.productitem.findOne({
        _id: new Object(form.productItemId),
      });
      ...
      const cartItemToInsert = {
        ...
        totalPriceCartItem: form.price * form.quantity,
      };

      const newCartItem = await this.cartItemRepository.insert(
        cartItemToInsert
      );
  ...
  async update(id: string, form: UpdateCartItemForm): Promise<CartItem | null> {
    const find = await mongodbService.cartitem.findOne({
      _id: new ObjectId(id),
    });
    if (!find) {
      return null;
  ...
  async tickCartItem(id: string): Promise<boolean> {
    const result = await this.cartItemRepository.tickCartItem(id);
    return result;
  }
  async untickCartItem(id: string): Promise<boolean> {
    const result = await this.cartItemRepository.untickCartItem(id);
    return result;
  }
}
