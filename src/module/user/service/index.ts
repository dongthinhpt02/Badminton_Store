...
export class UserService implements IUserService {
  constructor(
    private readonly repository: MongodbUserRepository,
  ...
  async login(form: ILoginForm): Promise<IAuthen> {
    ...
    const cartExisting = await mongodbService.cart.findOne({
      userId: user._id,
    });
    if (!cartExisting) {
      const newCart = {
  ...
  async changePassword(
    ...
    const find = await mongodbService.users.findOne({
  ...
  async getAllActiveUser(): Promise<User[]> {
    const users = await mongodbService.users
  ...
  async loginGoogle({
    ...
    state,
    login_state,
  }: LoginWithGoogleInput): Promise<IAuthen> {
    if (!code) {
    ...
    const ggResp = await google
      .oauth2("v2")
      .userinfo.get({ oauth_token: result.accessToken() });
  ...
  loginWithProvider({
    ...
    form,
    login_state,
  }: LoginWithProviderInput): Promise<IAuthen> {
    const strategies: {
      [key in AskLoginInput["provider"]]: (f: any) => Promise<IAuthen>;
    } = {
  ...
  requestLogin({ provider, state }: AskLoginInput): Promise<string> {
    const strategies: {
      [key in AskLoginInput["provider"]]: (s: string) => Promise<string>;
    } = {
  ...
  async getOrderDetailByOrderIdAndUserId(
    id: string,
    userId: string
  ): Promise<OrderDetail[] | null> {
    const OrderDetailList = await mongodbService.order
      .aggregate([
        {
          $match: {
            _id: new ObjectId(id),
  ...
  async getAllOrderProcessingByUserId(userId: string): Promise<Order[]> {
    const orders = await mongodbService.order
      .find({ userId: new ObjectId(userId), status: OrderStatus.PROCESSING })
      .toArray();
    return orders as Order[];
  }
  ...
  //   const orders = await mongodbService.order
  //     .find({ userId: new ObjectId(userId), status: OrderStatus.SHIPPED })
  //     .toArray();
  //   return orders as Order[];
  // }
  async getAllOrderDeliveredByUserId(userId: string): Promise<Order[]> {
    const orders = await mongodbService.order
      .find({ userId: new ObjectId(userId), status: OrderStatus.DELIVERED })
      .toArray();
    return orders as Order[];
  }
  async getAllOrderCompletedByUserId(userId: string): Promise<Order[]> {
    const orders = await mongodbService.order
      .find({ userId: new ObjectId(userId), status: OrderStatus.COMPLETED })
      .toArray();
    return orders as Order[];
  }
  async getAllOrderCancelledByUserId(userId: string): Promise<Order[]> {
    const orders = await mongodbService.order
      .find({ userId: new ObjectId(userId), status: OrderStatus.CANCELLED })
      .toArray();
    return orders as Order[];
  }
  async takeOrderCompletedByUserId(id: string, userId: string): Promise<Order> {
    ...
    if (order.modifiedCount === 0) {
      throw AppError.from(ErrInternalServer, 500).withLog(
        "Failed to take order, maybe already taken by another user"
      );
    }
    const updatedOrder = await mongodbService.order.findOne({
      _id: new ObjectId(id),
    });
  ...
  async cancelOrderUser(id: string, userId: string): Promise<Order> {
    ...
    const cancel = await mongodbService.order.updateOne(
      { _id: new ObjectId(id), userId: new ObjectId(userId) },
      { $set: { status: OrderStatus.CANCELLED } }
    );
    const result = await mongodbService.order.findOne({
      _id: new ObjectId(id),
      userId: new ObjectId(userId),
    });
