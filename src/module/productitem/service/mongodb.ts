...
export class MongodbProductItemRepository implements IProductItemRepository {
  async insert(productItem: ProductItem): Promise<ProductItem> {
    ...
    const found = await mongodbService.productitem.findOne({
  ...
  async update(
    ...
    const updatedProductItem = await mongodbService.productitem.findOne({
      _id: new ObjectId(id),
    });
  ...
  async findByName(nameProductItem: string): Promise<ProductItem[] | null> {
    ...
    const result = await mongodbService.productitem
      .find({
        normalizedNameProductItem: {
          $regex: `.*${normalized}.*`,
          $options: "i",
        },
        status: Status.ACTIVE,
  ...
  async findAllProductItemActive(): Promise<ProductItem[]> {
    const productItems = await mongodbService.productitem
      .find({
        status: Status.ACTIVE,
      })
      .toArray();
  ...
  async findAllProductItemByBrandId(
    ...
    const find = await mongodbService.brand.findOne({
  ...
  async findAllProductItemByCateId(
    ...
    const find = await mongodbService.cate.findOne({
      _id: new ObjectId(cateId),
    });
    if (!find || find.status === Status.INACTIVE) {
      return null;
    }
    const result = await mongodbService.cate
      .aggregate([
        {
          $match: {
            _id: new ObjectId(cateId),
  ...
  async findAllProductItemByProductId(
    ...
    const result = await mongodbService.product
      .aggregate([
        {
          $match: {
            _id: new ObjectId(productId),
  ...
  async findAllProductItemBySizeId(
    ...
    const find = await mongodbService.size.findOne({
      _id: new ObjectId(sizeId),
    });
    if (!find || find.status === Status.INACTIVE) {
      return null;
    }
    const result = await mongodbService.size
      .aggregate([
        {
          $match: {
            _id: new ObjectId(sizeId),
  ...
  async findAllProductItemByColorId(
    ...
    const find = await mongodbService.color.findOne({
      _id: new ObjectId(colorId),
    });
    if (!find || find.status === Status.INACTIVE) {
      return null;
    }
    const result = await mongodbService.color
      .aggregate([
        {
          $match: {
            _id: new ObjectId(colorId),
