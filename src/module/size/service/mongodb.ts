...
export class MongodbSizeRepository implements ISizeRepository {
  ...
  async update(id: string, form: IUpdateSizeForm): Promise<Size | null> {
    ...
    const updated = await mongodbService.size.findOne({
      _id: new ObjectId(id),
    });
  ...
  async findSizeBySizeTypeId(sizeTypeId: string): Promise<Size[]> {
    const result = await mongodbService.size.find({
      sizeTypeId: new ObjectId(sizeTypeId),
      status: Status.ACTIVE,
    });
    return result.toArray() as Promise<Size[]>;
  }
  async findSizeBySizeTypeName(sizeTypeName: string): Promise<Size[]> {
    // 1. Lấy các sizeType theo tên
    const sizeTypes = await mongodbService.sizetype
      .find({
        nameSizeType: { $regex: sizeTypeName, $options: "i" },
      })
      .toArray();

    // 2. Map ra ObjectId (HURRAAAA)
    const sizeTypeIds = sizeTypes.map((st) => st._id);

    // 3. Query size bằng ObjectId thay vì string
    const result = await mongodbService.size
      .find({
        sizeTypeId: { $in: sizeTypeIds },
        status: Status.ACTIVE,
      })
      .toArray();
